...
class Hypergraph:
    ...
    def add_node(self, node: Node) -> str:
        ...
        if MEMORY_TTL > 0:
            self.redis.expire(node_key, MEMORY_TTL)
        return node.id
    ...
    def get_node(self, node_id: str) -> Optional[Node]:
        ...
        main_data = {
            k: v for k, v in data.items() if not k.startswith("prop:")
        }
    ...
    def add_hyperedge(self, edge: Hyperedge) -> str:
        ...
        for node_id in edge.node_ids:
            ...
            if MEMORY_TTL > 0:
                self.redis.expire(node_edges_key, MEMORY_TTL)
